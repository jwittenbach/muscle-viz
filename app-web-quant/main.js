...
// turn array of [0, 1] rgb values into rgb(r, g, b) string
function makeRGB(arr) {
	var rgb = arr.map(function(val) {
		return Math.round(255 * val);
	});
	return "rgb(" + rgb[0] + "," + rgb[1] + "," + rgb[2] + ")";
}

// colormap lookup: input values between 0 and maxValue
function getColor(value) {
	var maxIndex = colormap.length - 1
	var color = colormap[Math.round(maxIndex * value / maxValue)];
	return makeRGB(color);
}

// make gradient style for color bar
function getGradient() {
	const colors = colormap.map(makeRGB).join(", ");
...
function setParamsFromSVG(svg) {
	...
	["left", "right"].map(function (side) {
		for (i=0; i<params.nMuscles; i++) {
			state.muscles.main[side].push("empty");
...
function setFullStrength() {
	["left", "right"].map(function(side) {
		for (i=0; i<params.nMuscles; i++) {
				if (state.muscles.main[side][i] == "empty") {
					setMuscle(side, i, maxValue);
				}
		}
	});
}
...
function makeChangeValue(side, row) {
	return function() {
		var value;
		// empty value means that value was removed
		if (this.value === "") value = "empty";
		else value = parseFloat(this.value);
		// any value that is not a number in [0, maxValue] is invalid
		if (isNaN(value) || value < 0 || value > maxValue) value = "invalid";
		setMuscle(side, row, value);
	}
}
...
function makeButtonBanks(svg) {
	...
	["left", "right"].map(function(side) {
		for (row=0; row<params.nMuscles; row++) {
			...
			inputField.className = "inputField " + side + "InputField";
			inputField.onchange = makeChangeValue(side, row);
			buttons.main[side].push(inputField);
			...
			if (side === "left") {
				bankRow.appendChild(inputField);
...
function makeMisc() {
	var gradientStyle = getGradient();
	var colorBar = document.getElementById("colorBar")
	colorBar.style.setProperty("background-image", gradientStyle, "");
}
...
function inputChange(side, row, value) {
	console.log(side);
	console.log(row);
	console.log(value);
}
...
var colormap =
	[[0.      , 0.135112, 0.304751],
	 ...
	 [0.995737, 0.909344, 0.217772]].reverse();
